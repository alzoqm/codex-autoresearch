#!/usr/bin/env python3
from __future__ import annotations

from autoresearch_artifacts import (
    append_rows,
    build_launch_manifest,
    build_runtime_payload,
    build_state_payload,
    clone_state_payload,
    compare_summary_to_state,
    log_summary,
    make_row,
    parse_log_metadata,
    parse_metadata_comment,
    parse_results_log,
    read_json,
    read_launch_manifest,
    read_runtime_payload,
    read_state_payload,
    require_consistent_state,
    row_to_dict,
    sync_state_payload_session_mode,
    sync_state_session_mode,
    write_json_atomic,
    write_results_log,
)
from autoresearch_core import (
    AUTORESEARCH_OWNED_BASENAMES,
    ENV_ASSIGNMENT_RE,
    EXEC_SCRATCH_ROOT,
    HEADER,
    LESSONS_FILE_NAME,
    LAUNCH_MANIFEST_NAME,
    MAIN_LABEL_RE,
    MAIN_STATUSES,
    REQUIRED_STATE_FIELDS,
    RUNTIME_LOG_NAME,
    RUNTIME_STATE_NAME,
    WORKER_LABEL_RE,
    AutoresearchError,
    LogRow,
    ParsedLog,
    append_description_suffix,
    command_is_executable,
    decimal_to_json_number,
    evaluate_required_label_gate,
    format_keep_gate_miss_suffix,
    format_description_with_labels,
    format_decimal,
    format_delta,
    improvement,
    normalize_labels,
    parse_decimal,
    split_labels_from_description,
    utc_now,
)
from autoresearch_paths import (
    GitStatusEntry,
    archive_path_to_prev,
    canonical_repo_root,
    cleanup_exec_state,
    default_exec_state_path,
    default_hook_context_path,
    default_launch_manifest_path,
    default_lessons_path,
    default_runtime_log_path,
    default_runtime_state_path,
    default_state_path,
    find_repo_root,
    git_status_entries,
    git_status_paths,
    has_git_repo,
    is_autoresearch_owned_artifact,
    is_local_session_metadata,
    lexical_abspath,
    parse_scope_patterns,
    path_is_allowed_worktree_change,
    path_is_in_scope,
    prev_archive_path,
    repo_local_skill_mirror_is_in_scope,
    resolve_repo_path,
    resolve_repo_relative,
    resolve_repo_managed_path,
    resolve_state_path,
    resolve_state_path_for_log,
    results_repo_root,
)
from autoresearch_repo_targets import (
    RepoTarget,
    build_repo_targets,
    format_repo_target_label,
    normalize_scope_text,
    normalize_repo_commit_map,
    parse_companion_repo_scope_specs,
    parse_repo_commit_specs,
    primary_scope_from_config,
    repo_commit_map_for_targets,
    repo_targets_from_config,
    resolve_repo_target_path,
    serialize_repo_targets,
)
from autoresearch_orchestration import (
    DEFAULT_EXPLORATION_RATIO,
    DEFAULT_STRATEGY_POLICY,
    EXPLORATION_SOURCE_CHOICES,
    SELECTION_MODE_CHOICES,
    STRATEGY_POLICY_CHOICES,
    apply_iteration_to_orchestration,
    clone_orchestration_summary,
    decide_selection_mode,
    empty_orchestration_summary,
    extract_orchestration_context,
    merge_orchestration_labels,
    normalize_exploration_ratio,
    normalize_exploration_sources,
    normalize_selection_mode,
    normalize_strategy_family,
    normalize_strategy_policy,
    orchestration_labels,
    orchestration_summary_from_rows,
)
from autoresearch_research import (
    append_jsonl,
    apply_research_state_updates,
    build_experiment_report_markdown,
    clone_research_state,
    default_corpus_path,
    default_experiment_reports_dir,
    default_hypothesis_registry_path,
    default_sources_summary_path,
    empty_hypothesis_registry,
    empty_research_state,
    hypothesis_record,
    load_json_or_default,
    normalize_identifier,
    normalize_research_config,
    normalize_research_mode,
    normalize_research_phase,
    normalize_positive_int,
    render_sources_summary,
    source_record,
    write_json,
)

__all__ = [
    "AUTORESEARCH_OWNED_BASENAMES",
    "AutoresearchError",
    "ENV_ASSIGNMENT_RE",
    "EXEC_SCRATCH_ROOT",
    "GitStatusEntry",
    "HEADER",
    "LAUNCH_MANIFEST_NAME",
    "LESSONS_FILE_NAME",
    "LogRow",
    "MAIN_LABEL_RE",
    "MAIN_STATUSES",
    "ParsedLog",
    "RepoTarget",
    "REQUIRED_STATE_FIELDS",
    "RUNTIME_LOG_NAME",
    "RUNTIME_STATE_NAME",
    "WORKER_LABEL_RE",
    "append_description_suffix",
    "append_rows",
    "archive_path_to_prev",
    "build_launch_manifest",
    "build_runtime_payload",
    "build_state_payload",
    "canonical_repo_root",
    "cleanup_exec_state",
    "clone_state_payload",
    "command_is_executable",
    "compare_summary_to_state",
    "decimal_to_json_number",
    "default_exec_state_path",
    "default_hook_context_path",
    "default_launch_manifest_path",
    "default_lessons_path",
    "default_runtime_log_path",
    "default_runtime_state_path",
    "default_state_path",
    "DEFAULT_EXPLORATION_RATIO",
    "DEFAULT_STRATEGY_POLICY",
    "EXPLORATION_SOURCE_CHOICES",
    "find_repo_root",
    "format_description_with_labels",
    "format_decimal",
    "format_delta",
    "format_keep_gate_miss_suffix",
    "git_status_entries",
    "git_status_paths",
    "has_git_repo",
    "improvement",
    "is_autoresearch_owned_artifact",
    "is_local_session_metadata",
    "lexical_abspath",
    "log_summary",
    "make_row",
    "merge_orchestration_labels",
    "normalize_labels",
    "normalize_exploration_ratio",
    "normalize_exploration_sources",
    "normalize_selection_mode",
    "normalize_strategy_family",
    "normalize_strategy_policy",
    "parse_decimal",
    "parse_log_metadata",
    "parse_metadata_comment",
    "parse_results_log",
    "evaluate_required_label_gate",
    "extract_orchestration_context",
    "apply_iteration_to_orchestration",
    "clone_orchestration_summary",
    "decide_selection_mode",
    "empty_orchestration_summary",
    "orchestration_labels",
    "orchestration_summary_from_rows",
    "split_labels_from_description",
    "parse_scope_patterns",
    "parse_companion_repo_scope_specs",
    "path_is_allowed_worktree_change",
    "path_is_in_scope",
    "prev_archive_path",
    "repo_local_skill_mirror_is_in_scope",
    "resolve_repo_path",
    "resolve_repo_relative",
    "read_json",
    "read_launch_manifest",
    "read_runtime_payload",
    "read_state_payload",
    "build_repo_targets",
    "format_repo_target_label",
    "normalize_scope_text",
    "normalize_repo_commit_map",
    "primary_scope_from_config",
    "parse_repo_commit_specs",
    "repo_targets_from_config",
    "repo_commit_map_for_targets",
    "resolve_repo_target_path",
    "serialize_repo_targets",
    "require_consistent_state",
    "resolve_repo_managed_path",
    "resolve_state_path",
    "resolve_state_path_for_log",
    "results_repo_root",
    "row_to_dict",
    "SELECTION_MODE_CHOICES",
    "STRATEGY_POLICY_CHOICES",
    "sync_state_payload_session_mode",
    "sync_state_session_mode",
    "utc_now",
    "write_json_atomic",
    "write_results_log",
    "append_jsonl",
    "apply_research_state_updates",
    "build_experiment_report_markdown",
    "clone_research_state",
    "default_corpus_path",
    "default_experiment_reports_dir",
    "default_hypothesis_registry_path",
    "default_sources_summary_path",
    "empty_hypothesis_registry",
    "empty_research_state",
    "hypothesis_record",
    "load_json_or_default",
    "normalize_identifier",
    "normalize_positive_int",
    "normalize_research_config",
    "normalize_research_mode",
    "normalize_research_phase",
    "render_sources_summary",
    "source_record",
    "write_json",
]
